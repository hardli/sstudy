...
//      }
		...
		CvSeq *pContour = NULL;        
	    //drawContours(gray1, contours, 0, Scalar(255), CV_FILLED);
...
int _tmain(int argc, _TCHAR* argv[])
{
	...
	IplImage *img=cvLoadImage("H:\\pic\\bmp\\20170710092316_1.bmp");//原始图  
	//IplImage *ximg = NULL;
 ...
 //   IplImage *grayimg=cvCreateImage(cvGetSize(ximg),8,1);//灰度图  
 //   cvCvtColor(ximg,grayimg,CV_BGR2GRAY);//彩色转灰度  
	IplImage *grayimg=cvCreateImage(cvGetSize(img),8,1);//灰度图  
    cvCvtColor(img,grayimg,CV_BGR2GRAY);//彩色转灰度
	cvSaveImage("E:\\pictest\\1.bmp",img);
    ...
    for(int i=0;i<squares.size();i++){
		...
		Mat src=cvLoadImage("E:\\pictest\\ruihuapic.jpg");//原始图 
		Mat roi_of_image = src(roi);

        char * filename = new char[100];
        sprintf(filename,"E:\\pictest\\ruihuapic1.jpg");
