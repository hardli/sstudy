...
//      }
        ...
        for (size_t i = 0; i < contours.size(); i++ ) {
            ...
            if (approx.size() == 4 && isContourConvex(Mat(approx))) {
                ...
                if (3500.0<area) {//当正方形面积在此范围内……，如果有因面积过大或过小漏检正方形问题，调整此范围。
//                  printf("area=%lg\n",area);
...
int _tmain(int argc, _TCHAR* argv[])
{
	...
	IplImage *img=cvLoadImage("E:\\car.png");//原始图  
	IplImage *ximg = NULL;
	CvSize czSize;              //目标图像尺寸
	//计算目标图像大小  
    czSize.width = img->width * 1;  
    czSize.height = img->height * 1;
    ...
    for(int i=0;i<squares.size();i++){
        ...
        sprintf(filename,"E:\\pictest\\car.png");
