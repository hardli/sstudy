...
int main(){
	...
	for(int n =17; n <18; n++)
	{
		...
		char imageName[60] = "E:\\pictest\\黄牌\\17_dst.jpg";
		char picFront2[20] = "E:\\pictest\\黄牌\\"; 
		char picEnd2[9] = "_dst_dst.jpg";
		char str2[4];
		sprintf(str2,"%d",n);
		...
		char str3[4];
		sprintf(str3,"%d",n);
		string st3 = string(picFront3) + string(str3) + "\\" + string(str3) + string(picEnd3);
		char imageBwName[50];
		...
		unsigned char R1 = 110,G1 = 110, B1 = 28; 
		int iLowH = 26;//黄色对应的HSV最小值
        int iHighH = 34; //黄色对应的HSV最大值
		...
		while ((srcImage = cvLoadImage(imageName,1)) != NULL)
		{
		    ...
		    cvtColor(srcImage, bwImage, COLOR_BGR2HSV);
			...
			for (i = 0; i< imageHeight;i++)
			{
				for (j = 0;j<imageWidth;j++)
				{
					...
					if (pixelB <iHighH && pixelB> iHighL )
					{
			...
			for (tempSeq = seq;tempSeq != NULL; tempSeq = tempSeq->h_next)
			{
				...
				if (area > 100 && area < 50000)
				{
					...
					if (long2Short> 2.0 && long2Short < 3.8 && rectDegree > 0.63 && rectArea > 100 && rectArea <50000)
					{
