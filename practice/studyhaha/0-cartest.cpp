...
int main(){
	...
	for(int n =1; n <101; n++)
	{
		...
		if ((srcImage = cvLoadImage(imageName,1)) != NULL)
		{
			...
			CvSize size = cvSize(40, 90);//区域大小
			Scalar color = Scalar( 0, 255, 0 ); 
			rectangle((Mat)srcImage,cvPoint(0, a[0]-90),cvPoint(size.width ,a[0]),color,2);
			cout<<imageName<<"\n\n";
			//cvShowImage("srcImage",srcImage);
			cvSaveImage(imageDstName, srcImage);//保存目标图像  
			//cvSetImageROI(srcImage, cvRect(0, a[0]-size.height, size.width, size.height));//设置源图像ROI  
