...
int main(){
	...
	for(int n =1; n <101; n++)
	{
		...
		while ((srcImage = cvLoadImage(imageNamee,1)) != NULL)
		{
			//cvShowImage("srcImage",srcImage); 
			cout<<imageNamee<<": "<<endl;
			...
			for (tempSeq = seq;tempSeq != NULL; tempSeq = tempSeq->h_next)
			{
				...
				if (area > 50 && area < 50000)
				{
					...
					if (long2Short> 1.5 && long2Short < 3.8 && rectDegree > 0.63 && rectArea > 50 && rectArea <500)
					{
			...
			imageCnt++;
			sprintf(imageNamee,"%d.jpg",imageCnt);
